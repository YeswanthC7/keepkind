...
- **Evidence-first outputs:** answers/receipts are generated from retrieved context; if context is insufficient, the system should say so rather than guess.
- **Traceability:** every response includes chunk/source identifiers so results can be audited.
- **Durable artifacts:** receipts are stored and retrievable (not ephemeral chat responses).
...
- `sources`: source metadata (type, uri/title, trust level, content hash)
- `chunks`: chunked content + metadata + `embedding` (pgvector)
- `receipts`: persisted decision receipts + citations + assumptions + lifecycle metadata
...
5. Server returns:
   - `answer`
   - `citations` (chunkId/sourceId/distance + optional indexing metadata)
   - `contextUsed` (implementation-dependent)

### D) Retrieve → Decision Receipt (auditable artifact)
1. Client requests a receipt for an item question.
2. Server retrieves top-k chunks (same as RAG retrieval).
3. Server calls Ollama chat with a strict receipt format:
   - `RECOMMENDATION: <maintain|repair|resell|recycle|keep>`
   - `RATIONALE: ...`
   - `ASSUMPTIONS: ...`
4. Server parses/enforces the strict format and persists a row in `receipts` containing:
   - question, recommendation, rationale
   - citations (JSONB; no chunk text)
   - assumptions (JSONB)
   - generation metadata (e.g., chat_model, embed_model, k_used, prompt_version)
   - lifecycle fields (created_at, deleted_at)
   - `receipt_version` per item (monotonically increasing; ignores soft-deleted rows)
5. Server returns the saved receipt + clean citations + generation metadata.

## Citations storage
Citations are stored as a clean JSON array (no chunk text), for example:
...
Reproducibility (re-run retrieval later)

Traceability back to original chunks and sources

Receipts lifecycle & retrieval behavior

Ordering: receipts are ordered by created_at (timestamptz, default now).

Versioning: receipt_version increments per item for non-deleted receipts.

Soft delete: deleting a receipt sets deleted_at (timestamptz).

Auditing: list endpoints support includeDeleted=true to return soft-deleted receipts.

Latest: a dedicated endpoint returns the most recent non-deleted receipt for an item.
...
GET /items/{itemId}/chunks/search?q=...&limit=... (keyword sanity check)
...
Receipts (create/read/list/export/lifecycle):

POST /items/{itemId}/receipt?q=...&k=... (persisted receipt)

GET /items/{itemId}/receipts?limit=...&offset=...&includeDeleted=false (returns count, total, receipts[])

GET /items/{itemId}/receipts/latest

GET /items/{itemId}/receipts/{receiptId} (item-scoped)

GET /receipts/{receiptId} (global)

GET /receipts/{receiptId}/export.md (global)

GET /items/{itemId}/receipts/{receiptId}/export.md (item-scoped)

DELETE /items/{itemId}/receipts/{receiptId} (soft delete: sets deleted_at)
...
ingestion → chunking → embeddings → vector retrieval → RAG answers

decision receipt generation + persistence

receipt listing/retrieval (pagination + total), latest receipt

receipt lifecycle (soft delete + includeDeleted auditing)

Markdown export (global + item-scoped)

generation metadata persisted with receipts (chat_model, embed_model, k_used, prompt_version)
EOF
